...
def get_crop_ODP(input_map, cali_M, flip = False, msize=200):
    M = cali_M
    ...
    imgae_ps = temp_result
    #imgae_ps = np.roll(imgae_ps, shift=(-1, 0), axis=(0, 1))
    return imgae_ps

def SLM_create_amp_ODP(amp_input):
    ...
    # min_map = F.interpolate(min_map.unsqueeze(0).unsqueeze(0), (RESIZE_SIZE, RESIZE_SIZE)).squeeze()
    # max_map = F.interpolate(max_map.unsqueeze(0).unsqueeze(0), (RESIZE_SIZE, RESIZE_SIZE)).squeeze()
    ...
    over_phase = F.interpolate(over_phase.unsqueeze(0).unsqueeze(0), (MASK_SIZE, MASK_SIZE)).squeeze()
    #over_phase = 0.5*(2*np.pi - over_phase)

    pad_size = 400
    phase_cali = F.pad(phase_cali.unsqueeze(0).unsqueeze(0), ((pad_size-RESIZE_SIZE)//2, (pad_size-RESIZE_SIZE)//2, (pad_size-RESIZE_SIZE)//2, (pad_size-RESIZE_SIZE)//2), mode=mode_type).squeeze()
    
    pad_size = 800
    phase_cali = F.pad(phase_cali.unsqueeze(0).unsqueeze(0), ((pad_size-400)//2, (pad_size-400)//2, (pad_size-400)//2, (pad_size-400)//2), mode=mode_type).squeeze()
    ...
    return SLM_input, over_phase

def SLM_create_phase_ODP(phase_input, over_phase):
    mode_type = 'replicate'
    MASK_SIZE = 200
