...
##### camera calibration #####
def cali_M(cam, cam1, eng, exposure_time0, exposure_time1):
    ...
    cali_1_M, cali_0_M = calibrate_M_all(cam,cam1,exposure_time0,exposure_time1)
...
for epoch in range(MAX_EPOCH):
    ...
    for b, (data, target) in enumerate(train_loader):
        if b % 10 == 0:
            cali_1_M, cali_0_M = cali_M(cam, cam1, eng, exposure_time0, exposure_time1)
            ...
            for b_test, (data, target) in enumerate(test_loader):
                ...
                for ii in range(layer_num):
                    ...
                    if ii != layer_num - 1:
                        ...
                        y_out = y_1* torch.exp(1j*np.pi*(1/(1+(1/torch.square(torch.abs(y_1))))))
        ...
        for i in range(data_batch.shape[0]):
            ...
            if b % EPOCH_SAVE == 0:
                ...
                torch.save(optimizer.state_dict(), os.path.join(debug_dir, 'opt_state_epoch_%03d.npy' % b))
            ...
            for ii in range(layer_num):
                ...
                if ii != layer_num - 1:
                    ...
                    y_1_list.append(y_1)
                    rate_d_list.append(F_RATE / torch.abs(y_out).max())
            ...
            exposureModePtr.SetIntValue(exposureModePtr.GetEntryByName("Timed").GetValue())
            exposureTimePtr = PySpin.CFloatPtr(nodemap.GetNode("ExposureTime"))
            exposureTimePtr.SetValue(exposure_timeb1)
            cam1.BeginAcquisition()
            ...
            for ii in range(layer_num-1,-1,-1):
                ...
                if ii != 0:
                    E_1 = torch.mul(torch.exp(1j*SLM_phase_list[ii]).detach(), Pke_temp)

                    ## nonlinear function 1
                    temp_E = 1/(1+(1/torch.square(torch.abs(y_1_list[ii]))))
                    P1 = E_1 * (1+1j*np.pi*(torch.square(torch.abs(y_1_list[ii])))*torch.square(temp_E)*(1/torch.square(torch.square(torch.abs(y_1_list[ii]))))) * torch.exp(1j*np.pi*temp_E)
                    P2 = -1j*np.pi*E_1.conj()*torch.square(y_1_list[ii].conj())*torch.square(temp_E)*(1/torch.square(torch.square(torch.abs(y_1_list[ii])))) *torch.exp(-1j*np.pi*temp_E)
                    ## nonlinear function 2
                    ...
                    E_1 = P1 + P2
                    rate_e_list.append(B_RATE / torch.abs(E_1).max())
                    E_1 = rate_shape * E_1 / torch.abs(E_1).max()

            Pke_list.reverse()
            
            rate_d_list = np.array(rate_d_list)
            rate_e_list = np.array(rate_e_list)
            rate_d_list_final = []
            rate_e_list_final = []
            for ii in range(layer_num):
                rate_d_list_final.append(np.prod(rate_d_list[:(ii+1)]))
                rate_e_list_final.append(np.prod(rate_e_list[:(ii+1)]))
            rate_e_list_final.reverse()

            for ii in range(layer_num):
                grad_temp = 2*torch.mul(1j * Pkd_list[ii].detach(), Pke_list[ii]).real.to(torch.float32)
                
                grad_temp = grad_temp / (rate_d_list_final[ii] * rate_e_list_final[ii])
