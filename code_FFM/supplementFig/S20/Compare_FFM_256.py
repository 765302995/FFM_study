...
layer_size_list = [256,256,256,64,64]

## forward data evaluation
def forward_data_network_eval(M_I, modu_tensor_list, SYYM_matrix_list, nonlinear_flag, model_bn, SYYM_matrix_add, SYYM_matrix_add_2):
    M_X_out = M_I
    for ii in range(5):
    ...
    M_X_out = torch.matmul(M_X_out, SYYM_matrix_add_2)

    return M_X_out

## forward data train
def forward_data_network(M_I, modu_tensor_list, SYYM_matrix_list, nonlinear_flag, model_bn, SYYM_matrix_add, SYYM_matrix_add_2):
    M_X_out = M_I
    for ii in range(5):
...
class BN_net(nn.Module):
    def __init__(self):
        ...
        self.bnlayer = nn.ModuleList([nn.BatchNorm1d(256),nn.BatchNorm1d(256),nn.BatchNorm1d(64),nn.BatchNorm1d(64),nn.BatchNorm1d(64)])
...
## propagation matrix
SYYM_matrix_add = torch.zeros((256,64)).to(torch.float32).to(device)
for ii in range(64):
    SYYM_matrix_add[ii*4:(ii+1)*4,ii] = 1

SYYM_matrix_add_2 = torch.zeros((64,8)).to(torch.float32).to(device)
for ii in range(8):
    SYYM_matrix_add_2[ii*8:(ii+1)*8,ii] = 1
...
train_dataset = NLOSdataMNIST_PCA_C(train=True)
test_dataset = NLOSdataMNIST_PCA_C(train=False)
...
for epoch in range(MAX_EPOCH):
    ...
    for b, (data, target) in enumerate(train_loader):
        ...
        x_abs = forward_data_network(data_batch, modu_tensor_list, SYYM_matrix_list, nonlinear_flag, model_bn, SYYM_matrix_add, SYYM_matrix_add_2)
    ...
    if epoch % 1 == 0:
        ...
        for b, (data, target) in enumerate(test_loader):
            ...
            x_abs = forward_data_network_eval(data_batch, modu_tensor_list, SYYM_matrix_list, nonlinear_flag, model_bn, SYYM_matrix_add, SYYM_matrix_add_2)
