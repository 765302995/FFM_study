...
phase_scatter = torch.from_numpy(np.load('phase_init_blur.npy'))
...
train_dataset = NLOSdatafocus(train=True)
test_dataset = NLOSdatafocus(train=False)

train_loader = torch.utils.data.DataLoader(train_dataset,
                                            batch_size=1, shuffle=True, **gpu_args)
...
for epoch in range(MAX_EPOCH):
    ...
    for b, (data, target) in enumerate(train_loader):
        ...
        exposureModePtr.SetIntValue(exposureModePtr.GetEntryByName("Timed").GetValue())
        exposureTimePtr = PySpin.CFloatPtr(nodemap.GetNode("ExposureTime"))
        exposureTimePtr.SetValue(exposure_time)  #exposure_time
        cam1.BeginAcquisition()
        ...
        E_field = 2*E_field / torch.abs(E_field).max()
    ...
    writer.add_scalar('correct', train_correct_all/len(train_loader), global_step=epoch)
    print(f"Train---Epoch {epoch}: loss {train_loss_all/len(train_loader):.6f}, accuracy {train_correct_all/len(train_loader)}.")

    grad_1_mean = grad_1_all / len(train_loader)
