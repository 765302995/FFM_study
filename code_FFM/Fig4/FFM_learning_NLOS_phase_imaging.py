...
def cal_plane_phase(shift_phase_pi2, shift_phase_pi, cali_0_M, cam, gt_plane_phase, eng2):
    #object_input
    ...
    success = eng2.set_img_1024('temp.bmp')
    time.sleep(0.2)
...
fpsEnablePtr.SetValue(True)
fpsPtr = PySpin.CFloatPtr(nodemap.GetNode("AcquisitionFrameRate"))
fpsPtr.SetValue(target_fps) #target_fps
exposureModePtr = PySpin.CEnumerationPtr(nodemap.GetNode("ExposureMode"))
exposureModePtr.SetIntValue(exposureModePtr.GetEntryByName("Timed").GetValue())
exposureTimePtr = PySpin.CFloatPtr(nodemap.GetNode("ExposureTime"))
exposureTimePtr.SetValue(exposure_time//2)
...
phase_scatter = torch.from_numpy(phase_scatter_init)
...
train_dataset = NLOSdata2i_IMG(train=True)
test_dataset = NLOSdata2i_IMG(train=False)
...
for epoch in range(MAX_EPOCH):
    ...
    for b, (data, target) in enumerate(train_loader):
        ...
        if b%1 == 0:
            shift = cal_plane_phase(shift_phase_pi2, shift_phase_pi, cali_0_M, cam, gt_plane_phase, eng2)

        for i in range(data_batch.shape[0]):
            target_temp = target_batch[i,0].to(torch.float64)
    ...
    if epoch % 10 == 0:
        ...
        print(f"Test---Epoch {epoch}: loss {test_loss_all/len(test_loader):.6f}.")